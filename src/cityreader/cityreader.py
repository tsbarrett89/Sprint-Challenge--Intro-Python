...
def cityreader(cities=[]):
    with open("src/cityreader/cities.csv") as city:
        ...
        for each in city_data:
            big_city = City(each[0], float(each[3]), float(each[4]))
            cities.append(big_city)
...
cityreader_stretch(int(input1.split(',')[0]), int(input1.split(',')[1]), int(input2.split(',')[0]), int(input2.split(',')[1]), cities)